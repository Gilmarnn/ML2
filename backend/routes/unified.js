...
router.get('/products', requireUser, async (req, res) => {
  ...
  params.push(limit);
  const result = await pool.query(`SELECT up.id,up.account_id,up.platform,up.platform_product_id,up.title,up.price,up.currency,up.stock,
    ...
    WHERE ${where} ORDER BY up.sold_quantity DESC,up.last_synced_at DESC NULLS LAST LIMIT $${params.length}`, params);
